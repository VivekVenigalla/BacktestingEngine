...
TEST_CASE("smaCross places a protective bracket once its buy order actually fills", "[smacross]") {
    ...
    //"bool a = false, b = false;" declares two variables of the same type
    //in one statement - equivalent to two separate "bool a = false;" and
    //"bool b = false;" lines
    ...
    //asserting these two flags at the end(rather than just relying on the
    //size()==2 check above) confirms it was specifically one stop AND one
    //limit order that got created - size()==2 alone would also pass if,
    //say, two stop orders had been created by mistake instead
    REQUIRE(foundStop);
    REQUIRE(foundLimit);
}
